    ...
    <main class="container mx-auto px-4 py-8 center">
        ...
        <section class="text-center mt-8 mb-5">
            <h1 class="text-[3rem] font-extrabold" style="color: #4574b8">
                UNDIAN KREDIT BERHADIAH TAHUN {{ date('Y')}}
            </h1>
            ...
            <div class="w-[70%] bg-white p-6 rounded shadow">
                ...
                <div class="box flex flex-col items-center justify-center gap-4 p-6">
                    ...
                    <div class="flex gap-4">
                        ...
                        <a href="{{ route('undi.kredit') }}" class="btn btn-warning text-white">
            ...
            <div class="card-body">
                ...
                <div class="modal-action">
                    <form method="dialog">
                        ...
                        <input type="hidden" name="winner_wilayah" id="winner_wilayah">
                        <input type="hidden" name="winner_status" id="winner_status">

                        <!-- if there is a button in form, it will close the modal -->
                        <button type="button" class="btn btn-primary" id="submitButton">Simpan</button>
                        <a href="{{ route('undi.kredit') }}" class="btn btn-error" id="resetButton">Reset</a>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        function createPemenangKredit() {
            ...
            const data = {
                ...
                wilayah: $('#winner_wilayah').val(),
                status: $('#winner_status').val(),
            };
            ...
            function getWinner(winner, id_hadiah) {
                ...
                axios.get(`${window.location.origin}/api/kupon-kredit/${winner}?id_hadiah=${id_hadiah}`)
                    .then(function(response) {
                        ...
                        $('#winner_alamat').html(data.jenis_kredit);
                        $("#win_wilayah").html(data.wilayah);
                        ...
                        $("#winner_wilayah").val(data.wilayah);
                        $("#winner_status").val(data.status);
                    })
