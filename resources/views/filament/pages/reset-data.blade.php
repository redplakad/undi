<x-filament-panels::page>
    ...
    @endif
    <form action="{{ route('reset.data') }}" method="post" class="bg-white space-y-4 p-4 border rounded-lg shadow-sm max-w-lg">
        @csrf
        <div class="p-4 bg-blue-100 border-l-4 border-blue-500 text-blue-700" role="alert" style="color:rgb(161, 98, 7);background-color:rgb(254, 252, 232);border:1px solid rgba(250,204,21,0.4);border-left:4px solid rgb(250,204,21);display:block;">
            ...
            <p>Dengan melakukan reset data, maka semua data pemenang yang sudah ada sebelumnya akan terhapus secara otomatis.</p>
            <br>
            <p>Tolong gunakan tool ini dengan hati-hati.</p>
        </div>
        
        <div class="flex flex-col">
            <label for="konfirmasi" class="mb-2 text-lg font-medium text-gray-700">Silahkan ketik OKE, untuk melanjutkan<sup class="text-danger-600 dark:text-danger-400 font-medium">*</sup></label>
            <input type="text" id="konfirmasi" name="konfirmasi" required class="p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
