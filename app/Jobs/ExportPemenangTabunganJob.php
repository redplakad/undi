...
class ExportPemenangTabunganJob implements ShouldQueue
{
    ...
    {
        $fileName = 'Pemenang_tabungan_' . now()->format('YmdHis') . '.csv';
        ...
        // Get data from model
        PemenangTabungan::chunk(2000, function ($PemenangTabungans) use (&$row, $file) {
            foreach ($PemenangTabungans as $Pemenang) {
                fputcsv($file, [
                    $Pemenang->id_peserta,
                    $Pemenang->id_hadiah,
                    $Pemenang->no_kupon,
        ...
        if ($status && $user) {
            $title = "Export Pemenang Tabungan Selesai";
            $message = "Proses export Pemenang Tabungan selesai, sebanyak ".number_format($row)." berhasil di export ke file csv.";
