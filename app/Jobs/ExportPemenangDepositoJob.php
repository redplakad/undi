...
class ExportPemenangDepositoJob implements ShouldQueue
{
    ...
    {
        $fileName = 'Pemenang_deposito_' . now()->format('YmdHis') . '.csv';
        ...
        // Get data from model
        PemenangDeposito::chunk(2000, function ($PemenangDepositos) use (&$row, $file) {
            foreach ($PemenangDepositos as $Pemenang) {
                fputcsv($file, [
                    $Pemenang->id_peserta,
                    $Pemenang->id_hadiah,
                    $Pemenang->no_kupon,
        ...
        if ($status && $user) {
            $title = "Export Pemenang Deposito Selesai";
            $message = "Proses export Pemenang Deposito selesai, sebanyak ".number_format($row)." berhasil di export ke file csv.";
